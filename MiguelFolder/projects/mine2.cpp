...
void reveal(int i, int j)
{
    ...
    if (isMine(i - 1, j - 1) && i > 0 && j > 0)
    {
        numMines++;
    }
    if (isMine(i + 1, j + 1) && i < height - 1 && j < width - 1)
    {
        numMines++;
    }
    if (isMine(i - 1, j + 1) && i > 0 && j < width - 1)
    {
        numMines++;
    }
    if (isMine(i + 1, j - 1) && i < height - 1 && j > 0)
    {
        numMines++;
    }
    if (isMine(i - 1, j) && i > 0)
